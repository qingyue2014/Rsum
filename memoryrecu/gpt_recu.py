...
def gpt_response_results(prompt, model_name):
    ...
    input_qs_token_l = len(encoding.encode(prompt))  # token num
    input_qs_word_l = len(prompt.split(" "))  # word num
    qs_w_t_ratio = input_qs_word_l / input_qs_token_l
    MaxLen, TarLen = args.window_size, args.target_size
    max_word_num = int((MaxLen - TarLen) * qs_w_t_ratio)
    new_prompt = " ".join(prompt.split(" ")[-max_word_num:])'''
    for i in range(100):
        try:
...
def run_memorybank(args, test_dataset, language='en'):
    ...
    for test_dial in tqdm(test_dataset):
        ...
        temp1 = {0:memory['summary'][0], 1:memory['summary'][1]}
        result1 = summarize_overall_prompt(temp1,language=language)
        memory1 = llm_client.generate_text_simple(prompt=result1,prompt_num=gen_prompt_num,language=language)

        temp2 = {0:memory['summary'][2], 1:memory['summary'][3]}
        result2 = summarize_overall_prompt(temp2,language=language)
        memory2 = llm_client.generate_text_simple(prompt=result2,prompt_num=gen_prompt_num,language=language)

        temp3 = {0:{'content':memory1}, 1:{'content':memory2}}
        result3 = summarize_overall_prompt(temp3,language=language)
        overall_person_prompt = summarize_overall_personality(list(memory['personality'].items()),language=language)
        memory['overall_history'] = llm_client.generate_text_simple(prompt=result3,prompt_num=gen_prompt_num,language=language)
        ...
        input_str = DEFINED_PROMPT.format_map({"history": memory['overall_history'], "persona": memory['overall_history'], "dialog": test_dial['window']})
    ...
    wfile= os.path.join("save_msc/memory", f"{args.operation}_{args.mode}_sid{args.session_id}.json")
...
def run_memorybank_response(args, test_dataset):
    file_path = os.path.join("save_msc/memory", f"infer_full_sid{args.session_id}.json")
